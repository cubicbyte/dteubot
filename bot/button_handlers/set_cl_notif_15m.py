...
async def handler(update: Update, context: CallbackContext):
    ...
    context._chat_data.set('cl_notif_15m', state)

    if state:
        context._chat_data.set('cl_notif_1m', False)

    if suggestion:
        await update.callback_query.answer(context._chat_data.get_lang()['alert.cl_notif_enabled_tooltip'].format(remaining='15'), show_alert=True)
