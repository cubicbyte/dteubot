...
def count_no_lesson_days(
        schedule: list[TimeTableDate],
        date: _date,
        direction_right=True) -> timedelta | None:
...
def create_message(context: ContextTypes.DEFAULT_TYPE, date: _date | str) -> dict:
    ...
    # If there is no lesson for the current day
        ...
        if skip_right is None:
            skip_right = date_end - date + timedelta(days=1)
        if skip_left is None:
            skip_left = date - date_start + timedelta(days=1)
        ...
        if skip_left.days > 1 or skip_right.days > 1:
            date_start_localized = get_localized_date(context, date - skip_left + timedelta(days=1))
            date_end_localized = get_localized_date(context, date + skip_right - timedelta(days=1))
            msg_text = lang['page.schedule.empty.multiple_days'].format(
                dateStart=date_start_localized,
                dateEnd=date_end_localized
            )
        # If no lessons for only one day
        else:
            msg_text = lang['page.schedule.empty'].format(date=get_localized_date(context, date))

    # Decide whether to show the "today" button,
    # and also decide the "next" and "previous" buttons day skip values
    if cur_day_schedule is not None:
