...
parser.add_argument('-o', '--output', type=str, help='Main bot data directory path')
parser.add_argument('path', type=str, nargs='*', help='Other bot data directory path to merge')

def merge_user_data(main_path: str, other_path: List[str]):
    logger.info('Merging user data')
...
def main(main_path: str, other_path: List[str]):
    ...
    logger.info('Bot data merging finished')
...
def _merge_data(main_path: str, other_path: List[str]):
    ...
    logger.info('Merging data into cache')
...
def _merge_user_latest_updates(main_path: str, other_path: List[str]):
    logger.info('Merging logs/telegram/users/.../latest_update.json')
    data_cache = {}
    ...
    logger.info('Flushing logs/telegram/users/.../latest_update.json')
...
def _merge_user_updates(main_path: str, other_path: List[str]):
    logger.info('Merging logs/telegram/users/.../updates.txt')
    data_cache = {}
    for path in other_path + [main_path]:
        path = os.path.join(path, 'logs', 'telegram', 'users')
        for user_id in os.listdir(path):
...
def _merge_chat_messages(main_path: str, other_path: List[str]):
    logger.info('Merging logs/telegram/chats/.../messages.txt')
    data_cache = {}
    for path in other_path + [main_path]:
    ...
    logger.info('Flushing logs/telegram/chats/.../messages.txt')
...
def _merge_chat_cb_queries(main_path: str, other_path: List[str]):
    logger.info('Merging logs/telegram/chats/.../cb_queries.txt')
    data_cache = {}
    for path in other_path + [main_path]:
        path = os.path.join(path, 'logs', 'telegram', 'chats')
        for chat_id in os.listdir(path):
    ...
    logger.info('Flushing logs/telegram/chats/.../cb_queries.txt')
    for chat_id, data in data_cache.items():
