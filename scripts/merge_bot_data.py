...
def merge_user_data(main_path: str, other_path: List[str]):
    """Merge user data from other bot data directories into main one"""
...
def merge_chat_data(main_path: str, other_path: List[str]):
    """Merge chat data from other bot data directories into main one"""
...
def merge_cache(main_path: str, other_path: List[str]):
    ...
    """Merge cache from other bot data directories into main one"""


def merge_logs(main_path: str, other_path: List[str]):
    """Merge logs from other bot data directories into main one"""
...
def main(main_path: str, other_path: List[str]):
    ...
    _logger.info('Merging finished in %ss', round(time.process_time(), 3))
    ...
    for path in other_path:
        for filename in os.listdir(path):
            ...
            if filename not in data_cache:
                main_filepath = os.path.join(main_path, filename)

                if not os.path.exists(main_filepath):
                    data_cache[filename] = data
                    continue

                with open(main_filepath, encoding='utf-8') as file:
                    data_cache[filename] = json.load(file)

            if data['_updated'] > data_cache[filename]['_updated']:
                data_cache[filename] = data
...
def _merge_chat(file, cache):
    ...
    for line in file:
        ...
        for i, line2 in enumerate(_cache):
            if timestamp > line2[0]:
...
def _get_latest_update_time(file):
...
def _merge_user_latest_updates(main_path: str, other_path: List[str]):
    ...
    for path in other_path + [main_path]:
        ...
        for user_id in os.listdir(path):
            with open(os.path.join(path, user_id, 'updates.txt'), encoding='utf-8') as file:
                update_time = _get_latest_update_time(file)
...
def _merge_user_updates(main_path: str, other_path: List[str]):
    ...
    for path in other_path + [main_path]:
        ...
        for user_id in os.listdir(path):
            ...
            with open(os.path.join(path, user_id, 'updates.txt'), encoding='utf-8') as file:
                _merge_chat(file, data_cache[user_id])

    for user_id, data in data_cache.items():
        filepath = os.path.join(main_path, 'logs', 'telegram', 'users', user_id, 'updates.txt')
...
def _merge_chat_messages(main_path: str, other_path: List[str]):
    ...
    for path in other_path + [main_path]:
        ...
        for chat_id in os.listdir(path):
            ...
            with open(os.path.join(path, chat_id, 'messages.txt'), encoding='utf-8') as file:
                _merge_chat(file, data_cache[chat_id])

    for chat_id, data in data_cache.items():
        filepath = os.path.join(main_path, 'logs', 'telegram', 'chats', chat_id, 'messages.txt')
...
def _merge_chat_cb_queries(main_path: str, other_path: List[str]):
    ...
    for path in other_path + [main_path]:
        ...
        for chat_id in os.listdir(path):
            ...
            with open(os.path.join(path, chat_id, 'cb_queries.txt'), encoding='utf-8') as file:
                _merge_chat(file, data_cache[chat_id])

    for chat_id, data in data_cache.items():
        filepath = os.path.join(main_path, 'logs', 'telegram', 'chats', chat_id, 'cb_queries.txt')
